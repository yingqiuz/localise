...
from localise.create_tracts import (
    create_target_lists,
    resolve_seed_path,
    parse_arguments,
...
class TestMainFunction:
    ...
    def test_main_both_hemispheres(self):
        ...
        with patch('localise.create_tracts.parse_arguments') as mock_parse:
            args = Mock()
            ...
            with patch('localise.create_tracts.create_tracts') as mock_create:
                main()
...
class TestEdgeCases:
    ...
    def test_empty_masks_directory(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            with patch('localise.create_tracts.check_fsl_environment', return_value='/usr/local/fsl'):
                with pytest.raises(FileNotFoundError):
                    create_tracts(
                        bpx=str(bpx_dir),
                        masks=str(masks_dir),
